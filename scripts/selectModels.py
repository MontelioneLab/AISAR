...
def print_usage():
    ...
    print("\nUsage: python selectModels.py <score_file> <dir_source> <selectedModels_dir> <NumberofModels>")
    print("\nArguments:")
    ...
    print("  <NumberofModels>     Number of selected models")
    print("\nExample:")
    print("  python selectModels.py scores.all ESmodels/ selectedModels/ 5")
    sys.exit(1)
...
scoresAll_df['Score'] = (np.sqrt(scoresAll_df['pTM']*(scoresAll_df['Recall']-scoresAll_df['Recall'].min())/(scoresAll_df['Recall'].max()-scoresAll_df['Recall'].min()+0.000001))+np.sqrt(scoresAll_df['pLDDT']/100*scoresAll_df['SCC']*-1))/2
...
for c in range(scoresAll_df['Cluster'].min(), scoresAll_df['Cluster'].max()+1):
    ...
    cluster_sorted[c] = cluster[c].sort_values(by=['Score'], ascending=False).head(numberModels)

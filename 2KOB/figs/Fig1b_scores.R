...
pNMR = (pNOE+pRCI)/2
pNMRs <- pNMR
pNMRs[pNMRs < 0.75] <- 0.75
mycol<-map2color(pNMRs,brewer.pal(10, "Spectral"))

par(fig = c(0, 0.26, 0, 0.34), new=TRUE)  # Position inside subplot
par(mar=c(4,4,2,0.5))

sp <- plot(df2$PCA1, df2$PCA2, mgp=c(2,1,0), main="pNMR", pch=20, cex=df2$pTM*df2$pTM*2, xlab="PC1", ylab="PC2", col=mycol) 

points(3.145,46.206, col="white", cex=1,font = 2)
points(32.713,-24.400, col="white",cex=1,font = 2)
points(31.784,9.476, col="white",cex=1,font = 2)
points(27.321,5.491, col="white",cex=1,font = 2)
points(-1.127,31.558, col="white",cex=1,font = 2)


par(fig = c(0.26, 0.27, 0.08, 0.34), new=TRUE)  # Position inside subplot
par(mar = c(0, 0, 2, 0))

score <- seq(min(pNMRs),max(pNMRs),length.out=11)
image(1, 1:10, t(matrix(1:10)), col = brewer.pal(10, "Spectral"), axes = FALSE)
axis(4, at = 0.5:10.5, labels = round(score, 2), las = 1)

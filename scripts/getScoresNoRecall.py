...
def print_usage():
    print("\nUsage: python getScores.py [scores.sc] [pLDDT.sc] [scc.sc] [cluster_pc_dm.csv]")
    print("\nArguments:")
...
pca_file = sys.argv[4] if len(sys.argv) > 4 else default_pca_file
...
with open(pLDDT_file) as f:
    for line in f:
        result = re.search(r"unrelaxed_([\w\_\d]+)\.pdb\s+(\d+\.\d+)", line)
        if result:
...
with open(pca_file) as f:
    ...
    for row in reader:
        ...
        result = re.match(r"(?:unrelaxed_)?([\w\d_]+)\.pdb", filename)
...
for name in pca1.keys():
    print(f"{name:<40} {float(pTM.get(name, -1.0)):7.3f} {float(pLDDT.get(name, -1.0)):7.3f} {float(pca1.get(name, -1.0)):7.3f} "
          f"{float(pca2.get(name, -1.0)):7.3f} {float(pca3.get(name, -1.0)):7.3f} 1 "
          f"1 {float(scc.get(name, 1.0)):7.3f} {int(grp.get(name, -1)):7}")

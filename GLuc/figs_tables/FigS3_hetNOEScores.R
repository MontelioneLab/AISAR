...
df<-read.table("scores_hetNOE.all", header=TRUE)
...
df2=df[order(df$SCC), ] #color by the score
mycol<-map2color(df2$SCC,brewer.pal(10, "Spectral"))

par(fig = c(0.33, 0.59, 0.34, 0.67), new=TRUE)  # Position inside subplot
par(mar=c(4,4,2,0.5))
plot(df2$PCA1, df2$PCA2, mgp=c(2,1,0), main="|hSCC|", pch=20, cex=df2$pTM*df2$pTM*2, xlab="PC1", ylab="PC2", col=mycol) 

par(fig = c(0.59, 0.60, 0.42, 0.67), new=TRUE)  # Position inside subplot
par(mar = c(0, 0, 2, 0))

score <- seq(min(df2$SCC),max(df2$SCC),length.out=11)
image(1, 1:10, t(matrix(1:10)), col = brewer.pal(10, "Spectral"), axes = FALSE)
axis(4, at = 0.5:10.5, labels = round(score, 2), las = 1)

###pLDDt*SCC

df2 <- df[order(sqrt(df$pLDDT/100 * df$SCC)), ]  # Sort by the transformed score
pRCI <- sqrt(df2$pLDDT/100 * df2$SCC)
mycol<-map2color(pRCI,brewer.pal(10, "Spectral"))

par(fig = c(0.66, 0.92, 0.34, 0.67), new=TRUE)  # Position inside subplot
par(mar=c(4,4,2,0.5))

plot(df2$PCA1, df2$PCA2, mgp=c(2,1,0), pch=20, main="phNOE", cex=df2$pTM*df2$pTM*2, xlab="PC1", ylab="PC2", col=mycol) 
...
### overall
df2=df[order(sqrt((df$Recall-min(df$Recall))/(max(df$Recall)-min(df$Recall))*df$pTM) + sqrt(df$pLDDT*df$SCC/100)), ] #color by the score
...
plot(df2$PCA1, df2$PCA2, mgp=c(2,1,0), main="phNMR", pch=20, cex=df2$pTM*df2$pTM*2, xlab="PC1", ylab="PC2", col=mycol) 
